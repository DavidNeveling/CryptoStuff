import message_space

def encrypt(message, message_space = message_space.PRINTABLE_CHARACTERS, shift = 1):
    encryption = ''
    for character in message:
        encryption += message_space[(message_space.find(character) + shift) % len(message_space)]
    return encryption

def decrypt(encryption, message_space = message_space.PRINTABLE_CHARACTERS, shift = 1):
    decryption = ''
